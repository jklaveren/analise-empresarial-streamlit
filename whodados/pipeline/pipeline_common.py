...
# Identifica quem esta baixando em vez do "curl/8.x" generico. Um cliente
# que se identifica e um download de dados abertos; e o oposto do padrao
# que dispara defesa anti-abuso (mesma tecnica ja validada no pipeline em
# levas -- ver pipeline_levas.py).
UA_RF = ("WhoDados-ETL/1.0 (dados abertos CNPJ; "
...
def _sondar_curl(url: str, extra: list[str], auth: str | None, timeout: int) -> str:
    ...
    cmd = ["curl", "-s", "-o", "/dev/null", "-w", "%{http_code}", "-L",
           "-A", UA_RF, "--max-time", str(timeout)] + extra
    if auth:
        cmd += ["-u", auth]
    cmd.append(url)
    try:
